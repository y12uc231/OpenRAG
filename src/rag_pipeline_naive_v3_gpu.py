...
retriever_model = BertModel.from_pretrained('bert-base-uncased').to(device)

generator_tokenizer = BartTokenizer.from_pretrained('facebook/bart-base')
generator_model = BartForConditionalGeneration.from_pretrained('facebook/bart-base').to(device)
...
def retrieve(query, knowledge_base, top_k=1):
    query_embedding = retriever_model(**retriever_tokenizer(query, return_tensors='pt').to(device)).pooler_output
    doc_embeddings = [retriever_model(**retriever_tokenizer(doc, return_tensors='pt').to(device)).pooler_output for doc in knowledge_base]
...
for epoch in range(num_epochs):
    ...
    for query, answer, knowledge_base in progress_bar:
        ...
        input_ids = generator_tokenizer.encode(input_text, return_tensors='pt').to(device)
        target_ids = generator_tokenizer.encode(answer, return_tensors='pt').to(device)
