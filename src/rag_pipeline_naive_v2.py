...
def retrieve(query, knowledge_base, top_k=1):
    ...
    top_embeddings = [doc_embeddings[i] for i in top_indices]
    
    return torch.stack(top_embeddings)
...
for epoch in range(num_epochs):
    ...
    for query, answer, knowledge_base in progress_bar:
        retrieved_embeddings = retrieve(query, knowledge_base)
        
        input_ids = generator_tokenizer.encode(query, return_tensors='pt')
        target_ids = generator_tokenizer.encode(answer, return_tensors='pt')
        
        outputs = generator_model(input_ids, labels=target_ids, encoder_outputs=(retrieved_embeddings.unsqueeze(0),))
        loss = outputs.loss
...
for query, answer, knowledge_base in dataset:
    ...
    encoder_outputs = ModelOutput(last_hidden_state=retrieved_embeddings.unsqueeze(0))
    output_ids = generator_model.generate(input_ids, encoder_outputs=encoder_outputs)
